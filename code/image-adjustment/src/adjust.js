...
export default class AdjustCanvasExt {
  ...
  constructor(option) {
    ...
    this.eventElement =
      this.imageCanvas.closest(`.${eventClassName}`) || this.imageCanvas
  ...
  _removeBlurEventLisener() {
    this.eventElement.removeEventListener(
      'mousedown',
      this._blurMousedownHandler
    )
    this.eventElement.removeEventListener(
      'mousemove',
      this._blurMousemoveHandler
    )
  ...
  _blurMouseupHandler(e) {
    ...
    if ([BLUR, MOSAIC].includes(this.imageOperation) && this._drawing && rect) {
      const { x, y, w, h } = rect
      if (
  ...
  _updateMoveCursor() {
    ...
    this._mouse.cursorIndex = inAnno
      ? CURSOR.GRAB
      : inImage
      ? CURSOR.CROSSHAIR
      : CURSOR.DEFAULT
  ...
  /* Crop */
  _setupCrop(option) {
    const { borderWidth, lineLen, canvasPadding } = option
  ...
  _cropMousemoveHandler(e) {
    ...
    if (!this._mouse.down) {
      ...
      for (let i = 0; i < this._mouseAreas.length; i++) {
        if (
          this._checkInPath(this._mouse.x, this._mouse.y, this._mouseAreas[i])
        ) {
    ...
    // update select
    if (this._mouse.down && this._mouse.cursorIndex < 9) {
  ...
  _removeCropEventListener() {
    ...
    this.imageCanvas.removeEventListener(
      'mousemove',
      this._cropMousemoveHandler
    )
    this.eventElement.removeEventListener('mouseup', this._cropMouseupHandler)
    this.imageCanvas.removeEventListener(
      'mousedown',
      this._cropMousedownHandler
    )
  }
  ...
  getAdjustedImage() {
    ...
    switch (this.imageOperation) {
      ...
      case BLUR:
      case MOSAIC:
        canvas2 = this._drawExistAnnotation(
          this.operatingImage.width,
          this.operatingImage.height,
          false
        )
  ...
  _checkAdujusted() {
    switch (this.imageOperation) {
      case CROP:
        ...
        const { x, y, w, h } = this._originImageRect
        this.imageAdjusted = !(sx === x && sy === y && sw === w && sh === h)
        break
      case ROTATE:
      ...
      case MOSAIC:
        this.imageAdjusted =
          ...
          this._annotations.some((anno) => anno.adjust)
      default:
        break
    }
  }
  ...
  _updateCanvas() {
    ...
    if ([BLUR, MOSAIC].includes(this.imageOperation)) {
      const canvas = this._drawExistAnnotation(
        this._originImageRect.w,
        this._originImageRect.h
      )
